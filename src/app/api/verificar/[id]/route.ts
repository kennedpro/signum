...
export async function GET(
  ...
  const checks = sigs.map((s) => {
    ...
    return {
      slot: s.slot,
      // Minimización de datos personales: la verificación pública acredita QUIÉN firmó
      // (nombre y cargo) sin exponer identificadores completos ni correo.
      signer: {
        name: s.signerName,
        cargo: s.signerCargo,
        cedula: s.signerCedula ? `•••${s.signerCedula.slice(-3)}` : null,
      },
