...
import { hit, reset, tooMany, RATE_LOGIN } from "@/lib/rate-limit";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Anti fuerza bruta: 6 intentos por IP+usuario cada 10 min; bloqueo de 15 min.
    const rateKey = `login:${ip ?? "local"}:${login.toLowerCase()}`;
    const rate = hit(rateKey, RATE_LOGIN);
    if (!rate.ok) {
      await safeLog([
        { documentId: null, event: "acceso_bloqueado", result: "fail", actorName: login, actorEmail: null, detail: `Límite de intentos superado · reintento en ${rate.retryAfterSec}s`, ip, userAgent: ua },
      ]);
      return tooMany(rate.retryAfterSec, "Demasiados intentos de acceso. Espere unos minutos e inténtelo de nuevo.");
    }
    ...
    reset(rateKey); // acceso correcto: se limpia el contador de intentos
