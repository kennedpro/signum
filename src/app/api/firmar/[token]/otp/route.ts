...
import { hit, tooMany, RATE_TOKEN, clientIp } from "@/lib/rate-limit";
...
export async function POST(
  ...
  {
    // Anti enumeración/fuerza bruta sobre enlaces de firma: 20 intentos por IP+token cada 10 min.
    const rl = hit(`token:${clientIp(await headers())}:${token.slice(0, 12)}`, RATE_TOKEN);
