...
import { ArrowDown, Archive, Forward, Loader2, MessageSquareText, Send, X } from "lucide-react";
...
/**
 * Barra superior del documento: volver al repositorio + botón «Comentarios»
 * (centrado a la altura del documento) + UID. Al pulsar, el hilo se despliega
 * justo debajo de la barra, sin desplazar la hoja lateralmente.
 */
export function DocTopBar({
  backHref = "/documentos",
  backLabel = "REPOSITORIO",
  uid,
  thread,
}: {
  backHref?: string;
  backLabel?: string;
  uid?: string;
  thread: Omit<Parameters<typeof MessageThread>[0], "className">;
}) {
  const [open, setOpen] = useState(false);
  const count = thread.initialMessages.length;

  // Cierre con Escape y bloqueo del scroll de fondo mientras el pop-up está abierto
  useEffect(() => {
    if (!open) return;
    ...
    <>
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          aria-expanded={open}
          className={cn(
            "inline-flex items-center gap-2 rounded-md border px-3.5 py-1.5 font-mono text-[11px] font-semibold transition",
            open ? "border-neon/50 bg-neon/10 text-neonsoft" : "border-line bg-panel/70 text-slate-300 hover:border-neon/40 hover:text-neonsoft"
          )}
          title="Ver y añadir comentarios del documento"
        >
          <MessageSquareText className="h-3.5 w-3.5 text-neon" />
          COMENTARIOS
          <span className={cn("rounded-full px-1.5 py-0.5 font-mono text-[9px] font-bold", count > 0 ? "bg-neon/15 text-neon" : "bg-line text-slate-500")}>
            {count}
          </span>
        </button>
        {uid ? (
          <p className="hidden font-mono text-[10px] uppercase tracking-[0.16em] text-slate-600 sm:block">UID {uid}</p>
        ) : (
      ...
      {open && (
        <div
          className="msg-popup__backdrop no-print"
          role="presentation"
          onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}
        >
          <div className="msg-popup" role="dialog" aria-modal="true" aria-label="Comentarios del documento">
            <button type="button" onClick={() => setOpen(false)} aria-label="Cerrar" className="msg-popup__close">
              <X className="h-4 w-4" />
            </button>
            <div className="msg-popup__body">
              <MessageThread {...thread} className="msg-popup__panel" />
            </div>
          </div>
        </div>
      )}
