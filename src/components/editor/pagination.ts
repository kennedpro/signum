...
function compute(view: import("@tiptap/pm/view").EditorView, opts: PgState["opts"]): DecorationSet {
  ...
  for (let i = 0; i < measured.length; i += 1) {
    ...
    if (!flows && !ignoreOverflow && top + b.height > end + TOLERANCE_PX) {
    ...
    const run = (view: import("@tiptap/pm/view").EditorView) => {
      ...
      const sig = (set: DecorationSet) =>
        set
          ...
          .map(
            (d) =>
              `${d.from}:${(d as unknown as { type: { attrs?: { style?: string } } }).type.attrs?.style ?? ""}`
          )
          .join("|");
    ...
    return [
      new Plugin<PgState>({
        ...
        view(view) {
          ...
          ro.observe(view.dom);
          const paper = view.dom.closest?.(".wd-paper");
